...
const { ErrorServer, ErrorUserInput } = require("../../utils/helpers/error.helper");
...
class UserModel {
  ...
  async findOne(id) {
    try {
      const result = await db.select("id", "fullname", "email", "gender", "created_at", "updated_at").table(this.tableName).where("id", id).limit(1);
  ...
  async findOneWithEmail(email) {
    try {
      const result = await db.select("id", "fullname", "email", "gender", "created_at", "updated_at").table(this.tableName).where("email", email).limit(1);
  ...
  async find(
    ...
    try {
      let query = db.select("id", "fullname", "email", "gender", "created_at", "updated_at").table(this.tableName);
